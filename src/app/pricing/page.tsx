...
export default function PricingPage() {
  ...
  const [loading, setLoading]       = useState<'trial' | 'yearly' | 'lifetime' | 'portal' | null>(null)
      ...
      <section className="pb-24 px-4">
        <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-6 items-stretch">
          ...
          <div className="rounded-3xl p-8 flex flex-col gap-6 relative overflow-hidden"
            ...
            <div>
              <p className="text-xs font-bold uppercase tracking-widest mb-2" style={{ color: 'var(--muted-foreground)' }}>Free trial</p>
              <div className="flex items-baseline gap-1">
                <span className="text-5xl font-extrabold" style={{ color: 'var(--foreground)' }}>€0</span>
                <span className="text-sm" style={{ color: 'var(--muted-foreground)' }}>today</span>
              </div>
              <p className="text-sm mt-2" style={{ color: 'var(--primary)' }}>Then €29.99/year after 3 days</p>
            </div>
                  ...
                  onClick={async () => {
                    ...
                    try {
                      const res = await fetch('/api/stripe/checkout', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({ plan: 'yearly', userId: user.id }),
                      })
                      const { url, error } = await res.json()
                      if (error) throw new Error(error)
                      sessionStorage.setItem('stripe_redirect', '1')
